...
<div class="space-y-12">
  ...
  <p>
    I took the lead on choosing the technical stack for Netflix's Chef's Table
    e-commerce platform, settling on Remix v2 paired with Shopify's Hydrogen
    framework and a custom Sanity CMS build. The decision came after
    establishing a proof of concept that demonstrated how these technologies
    could work together to create a headless commerce experience that provided
    us full control over the UX and design. Remix's server-side rendering
    capabilities combined with Hydrogen's commerce-specific tooling seemed like
    the right foundation for what we were trying to build.
  </p>
  ...
  <p>
    ...
    cart management were supposed to work. I implemented custom data loaders
    that satisfied Hydrogen's interface without overloading the system with
    unnecessary requests, which helped us maintain good performance while
    working within the framework's expectations for data fetching patterns.
  </p>
  ...
  <p>
    ...
    data directly into our Shopify data structure to keep everything synced.
    This meant designing a flexible schema system that could handle everything
    from product showcases to editorial content while maintaining the connection
    between our CMS and commerce data. The direct integration approach
    eliminated the need for separate sync processes and kept our data
    consistent.
  </p>
